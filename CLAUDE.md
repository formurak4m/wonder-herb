...
Reflects the actual repo as of 15 September 2026 (branch `rebuild/editor`). Where this file and the code disagree, the code wins. Fix this file.
...
- **Public pages:** 18 pages: 16 hand-coded static HTML files plus 2 pre-rendered (`產品介紹.html` and `常見問題.html`, rendered from `data/pages/products.json` and `faq.json`; their hand-coded originals are kept in `legacy/`).
...
- **Phase 9: two pages DONE (15 Sep 2026), awaiting owner review of page two.** 產品介紹 (`8d29617`) and 常見問題 (`fc8c6e8`) are live as pre-rendered pages; their originals are retired to `legacy/`, which carries noindex and is disallowed in robots.txt. Both visual diffs against the 15 Sep baseline are fully reconciled. Status, actual cost per page and the Phase 9 estimate are under P9-T1 in `BUILD_TASKS.md`. Pages whose original is in `legacy/` are written to the root by `npm run render`; never hand-edit them.
...
- **Content as data**: `data/products.json` (per-language titles/descriptions, finding 19), `data/cases.json`, `data/faq.json`, `data/homepage.json`, `data/inventory.csv`, and page trees in `data/pages/<slug>.json` (`products`, `faq`).
...
- **Sections** (`sections/`): 11 components shared by the editor and the renderer (page-header, hero, text-block, product-grid, product-detail, gallery, related-products, contact-cards, cta-band, faq-accordion, callout).
- **Behaviour for pre-rendered pages** (`assets/site.js` shared, plus `assets/behaviour/quick-view.js`):
  - `site.js`: cart, badge, phone menu, language switcher;
...
- **Test suite**: `npm run test:all` = 14 suites, 943 passing checks, green (15 Sep 2026).
...
1. **Content is rendered client-side on 16 of the 18 pages.** Those are static shells that `fetch('./data/*.json')` and build content in the browser, so crawlers mostly see an empty shell. 產品介紹 and 常見問題 are pre-rendered (content in the HTML). Nothing from this branch is deployed until it merges to `main`.
2. **The client cannot yet add or reorder sections.** The Puck editor and section model exist locally, but 16 of 18 page layouts are still hand-coded, and the editor isn't reachable by the client until Phase 11.
...
/                         public static site + config
  ...
  *.xlsx                  real sales/inventory spreadsheets - GITIGNORED, customer PII
  ...
  data/                   products.json, cases.json, faq.json, homepage.json, inventory.csv
    pages/                page trees, one per migrated page (products.json, faq.json)
  assets/                 published CSS + behaviour for pre-rendered pages
    ...
    behaviour/            per-section behaviour (quick-view.js)
  sections/               11 section components, shared by Puck AND the renderer
