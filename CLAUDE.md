...
- **The repo is PUBLIC.**
  ...
  - **Invented star ratings and reviews** (some built from patient case details) were in the JSON-LD of `main` and served publicly by this repo's GitHub Pages copy (`formurak4m.github.io/wonder-herb`); the client's Wix site never had them. Stripped from this branch (`7a918b6`), and `test:seo` fails any rating data. The `main` fix is `6503a01`, pushed only with owner approval. Recommendation: unpublish the Pages copy (finding 26).
