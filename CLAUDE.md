...
- **Content as data**: `data/products.json` (per-language titles/descriptions, finding 19), `data/cases.json`, `data/faq.json`, `data/homepage.json`, `data/inventory.csv`, and page trees in `data/pages/<slug>.json` (one so far: `products`).
...
- **Sections** (`sections/`): 10 components shared by the editor and the renderer (page-header, hero, text-block, product-grid, product-detail, gallery, related-products, contact-cards, cta-band, faq-accordion).
...
- **The repo is PUBLIC.**
  ...
  - **Finding 26 (CRITICAL):** invented star ratings, plus patients' cases (some the client's own case-study patients, diagnoses including cancer) published as five-star product reviews. They were in the JSON-LD of `main` and served by this repo's GitHub Pages copy from 4 Jun to 15 Sep 2026, and never on the client's Wix site as far as was checked. Removed from `main` (`6503a01`, pushed, live copy verified clean) and this branch (`7a918b6`, `d2ee9c4`). `test:seo` fails any rating data. The owner is unpublishing Pages and checking Google/Bing. **No health claim on the current pages can be presumed to be the client's own words**: trace each to a client-approved source before a migrated page publishes it (P13G-T2 claims provenance, applied per page).
...
/                         public static site + config
  ...
  *.xlsx                  real sales/inventory spreadsheets - GITIGNORED, customer PII
  ...
  data/                   products.json, cases.json, faq.json, homepage.json, inventory.csv
    pages/                page trees, one per migrated page (products.json)
  assets/                 published CSS + behaviour for pre-rendered pages
    chrome.css page-products.css   GENERATED by scripts/extract-css.js
    site.js               shared behaviour: cart, badge, phone menu, language switcher
    behaviour/            per-section behaviour (quick-view.js)
  sections/               10 section components, shared by Puck AND the renderer
