...
- **Public pages:** 18 pages: 10 hand-coded static HTML files plus 8 pre-rendered (產品介紹, 常見問題, 聯絡我們, 小册子, 有效成份檢測, 微信發表文章, 研究報告, 典型病例 — one tree each in `data/pages/`; their hand-coded originals are kept in `legacy/`).
...
- **Sections** (`sections/`): 14 components shared by the editor and the renderer (page-header, hero, text-block, product-grid, product-detail, gallery, related-products, contact-cards, cta-band, faq-accordion, callout, card-grid, map-embed, case-list).
- **Behaviour for pre-rendered pages** (`assets/site.js` shared, plus `assets/behaviour/quick-view.js`):
  - `site.js`: cart, badge, phone menu, language switcher; per-section files add quick view (product-grid) and the case filter (case-list);
...
1. **Content is rendered client-side on 10 of the 18 pages.** Those are static shells that `fetch('./data/*.json')` and build content in the browser, so crawlers mostly see an empty shell. The 8 migrated pages are pre-rendered (content in the HTML), including 典型病例, whose 15 case studies used to exist only after JavaScript ran. Nothing from this branch is deployed until it merges to `main`.
2. **The client cannot yet add or reorder sections.** The Puck editor and section model exist locally, but 10 of 18 page layouts are still hand-coded, and the editor isn't reachable by the client until Phase 11.
...
Known issues are logged in `docs/FINDINGS.md` (29 findings so far), each with the phase that owns its fix. The index table at the top shows which are fixed. **Do not fix a finding out of its phase** unless the owner says so.
...
/                         public static site + config
  ...
  *.xlsx                  real sales/inventory spreadsheets - GITIGNORED, customer PII
  ...
  data/                   products.json, cases.json, faq.json, homepage.json, inventory.csv
    pages/                page trees, one per migrated page (8 so far)
  assets/                 published CSS + behaviour for pre-rendered pages
    ...
    behaviour/            per-section behaviour (quick-view.js, case-list.js)
