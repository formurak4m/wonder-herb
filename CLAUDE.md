...
- **Public pages:** 18 pages: 3 hand-coded static HTML files plus 15 pre-rendered (the homepage, every content page and all six product detail pages — one tree each in `data/pages/`; their hand-coded originals are kept in `legacy/`). The three left are `product.html` / `購物車.html` / `account.html`, which are out of Phase 9 scope by decision.
...
- **Content as data**: `data/products.json` (per-language titles/descriptions, finding 19), `data/cases.json`, `data/faq.json`, `data/homepage.json`, `data/inventory.csv`, and page trees in `data/pages/<slug>.json` (15, one per migrated page).
...
- **Sections** (`sections/`): 15 components shared by the editor and the renderer (page-header, hero, text-block, product-grid, product-carousel, product-detail, gallery, related-products, contact-cards, cta-band, faq-accordion, callout, card-grid, map-embed, case-list). `card-grid` carries 7 variants, `callout` and `text-block` 4, `cta-band` 3: a variant is always markup that exists on the live site, never an invention.
- **Behaviour for pre-rendered pages** (`assets/site.js` shared, plus 7 files in `assets/behaviour/`):
  - `site.js`: cart, badge, phone menu, language switcher; per-section files add quick view (product-grid), the case filter (case-list), the buy panel (product-detail), the gallery thumbnails, the homepage hero carousel and its 3D models, the product carousel and the video band;
  - loaded by `renderer/template.js` according to the tree's section types, keyed by `type` or `type/variant` (a behaviour may belong to one variant only), with any third-party libraries it needs declared as `deps` ahead of it.
...
- **Test suite**: `npm run test:all` = 15 suites, 1,496 passing checks, green (16 Sep 2026).
...
1. **Content is rendered client-side on 3 of the 18 pages.** Those are static shells that `fetch('./data/*.json')` and build content in the browser, so crawlers mostly see an empty shell. The 15 migrated pages are pre-rendered (content in the HTML) — including 典型病例, whose 15 case studies used to exist only after JavaScript ran, and 產品_憶活素, whose medical citations were injected into an empty box. Nothing from this branch is deployed until it merges to `main`.
2. **The client cannot yet add or reorder sections.** The Puck editor and section model exist locally, but 3 of 18 page layouts are still hand-coded, and the editor isn't reachable by the client until Phase 11.
...
Known issues are logged in `docs/FINDINGS.md` (36 findings so far), each with the phase that owns its fix. The index table at the top shows which are fixed. **Do not fix a finding out of its phase** unless the owner says so.
...
/                         public static site + config
  ...
  *.xlsx                  real sales/inventory spreadsheets - GITIGNORED, customer PII
  ...
  data/                   products.json, cases.json, faq.json, homepage.json, inventory.csv
    pages/                page trees, one per migrated page (15)
  assets/                 published CSS + behaviour for pre-rendered pages
    ...
    behaviour/            per-section behaviour (quick-view, case-list, product-detail, gallery,
                          home-hero, product-carousel, video-band)
  sections/               15 section components, shared by Puck AND the renderer
...
- **A heavy or externally hosted asset is a TREE FIELD, never markup.** The homepage carries its
  four `.glb` model URLs as per-slide fields and its background video as `videoUrl`, so the Phase 10
  move to R2 - and the death of the Wix CDN at cutover (finding 1) - is a data edit and a re-render,
  not a code change. A behaviour file never names a file: it reads `data-model` from the DOM.
- **A 3D model degrades to a photograph.** There is NO `<model-viewer>` on this site: the homepage
  hand-rolls three.js + GLTFLoader + OrbitControls, so there is no built-in poster. A model slide
  pre-renders the product's own photo inside the container the loader replaces, and the loader must
  not clear that container until the model is actually in the scene.
